from flask import Flask, render_template, request, redirect, url_for, send_file, flash from datetime import datetime, timedelta from supabase import create_client, Client import fitz  # PyMuPDF import os
...
PDF_OUTPUT_FOLDER = 'static/pdfs' PLANTILLA_PDF = 'Guerrero.pdf'

================= FUNCIONES DE FOLIOS =================

def cargar_folio(): response = supabase.table('folios_control').select('ultimo_folio').eq('id', 1).single().execute() return response.data['ultimo_folio']

def actualizar_folio(nuevo_folio): supabase.table('folios_control').update({'ultimo_folio': nuevo_folio}).eq('id', 1).execute()

def siguiente_folio(folio_actual): letras = folio_actual[:2] numeros = int(folio_actual[2:]) if numeros < 9999: numeros += 1 else: letras = incrementar_letras(letras) numeros = 0 nuevo_folio = f"{letras}{numeros:04d}" actualizar_folio(nuevo_folio) return nuevo_folio
...
def guardar_en_supabase(folio, tipo, marca, linea, año, serie, motor, color, contribuyente, fecha_expedicion, fecha_vencimiento): data = { "folio_generado": folio, "tipo_vehiculo": tipo, "marca": marca, "linea": linea, "anio": año, "serie": serie, "motor": motor, "color": color, "contribuyente": contribuyente, "fecha_expedicion": fecha_expedicion, "fecha_vencimiento": fecha_vencimiento } supabase.table("permisos_guerrero").insert(data).execute()

def generar_pdf(folio, tipo_vehiculo, marca, linea, año, serie, motor, color, contribuyente, fecha_expedicion, fecha_vencimiento): doc = fitz.open(PLANTILLA_PDF) page = doc[0]
...
@app.route('/panel', methods=['GET']) def panel(): buscar = request.args.get('buscar', '') registros = supabase.table('permisos_guerrero').select('*').execute().data
...
@app.route('/editar/<folio>', methods=['GET', 'POST']) def editar(folio): if request.method == 'POST': tipo_vehiculo = request.form['tipo_vehiculo'].upper() marca = request.form['marca'].upper() linea = request.form['linea'].upper() año = request.form['año'].upper() serie = request.form['serie'].upper() motor = request.form['motor'].upper() color = request.form['color'].upper() contribuyente = request.form['contribuyente'].upper()
...
@app.route('/eliminar/<folio>') def eliminar(folio): supabase.table('permisos_guerrero').delete().eq('folio_generado', folio).execute() flash('Registro eliminado exitosamente.', 'success') return redirect(url_for('panel'))
