...
# ---------------- FUNCIONES DE FOLIO ----------------
def cargar_folio():
    """Toma el último folio de Supabase o devuelve 'AA0001' si no hay ninguno."""
    resp = (
        supabase
    ...
    if datos and len(datos) > 0:
        return datos[0]["folio"]
    return "AA0001"

def siguiente_folio(folio_actual: str) -> str:
    """Incrementa 'AA0001' → 'AA0002' … 'AA9999' → 'AB0001', etc."""
...
def formulario():
    if 'usuario' not in session:
        return redirect(url_for('login'))
    if request.method == 'POST':
        # Leemos los campos tal como vienen en tu HTML
        marca         = request.form['marca'].upper()
        linea         = request.form['linea'].upper()
        anio          = request.form['anio'].upper()           # ← sin tilde
        ...
        # Días de validez (30/60/90)
        dias_validez  = int(request.form.get('validez', 30))
...
def consultar():
    if request.method == 'POST':
        ...
        flash("Folio no encontrado o sin PDF", "danger")
    return render_template('reimprimir.html')

@app.route('/descargar/<folio>')
def descargar(folio):
    pth = os.path.join(PDF_OUTPUT_FOLDER, f"{folio}.pdf")
    return send_file(pth, as_attachment=True) if os.path.exists(pth) else ("Archivo no existe", 404)
...
def renovar(folio):
    # Leer registro, verificar vencimiento y crear uno nuevo
    if not os.path.exists(REGISTRO_FILE):
    ...
    with open(REGISTRO_FILE, 'r', encoding='utf-8') as f:
        for ln in f:
            ...
            if len(parts) == 10 and parts[0] == folio:
                fecha_vieja = datetime.strptime(parts[9], "%d/%m/%Y")
                if datetime.now() >= fecha_vieja:
                    datos = parts
                break
    if not datos:
        flash("Aún vigente o no existe", "warning")
        return redirect(url_for('listar'))

    _, m, l, a, s, mo, c, co, _, _ = datos
    ...
    flash(f"Renovado: {nuevo}", "success")
