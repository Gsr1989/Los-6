...
def guardar_en_supabase(folio, tipo, marca, linea, año, serie, motor, color, contribuyente, fecha_expedicion): data = { "folio": folio, "tipo_vehiculo": tipo, "marca": marca, "linea": linea, "año": año, "serie": serie, "motor": motor, "color": color, "contribuyente": contribuyente, "fecha_expedicion": fecha_expedicion } supabase.table("permisos_guerrero").insert(data).execute()

def generar_pdf(folio, tipo_vehiculo, marca, linea, año, serie, motor, color, contribuyente, fecha_expedicion, fecha_vencimiento): tipo_texto = obtener_texto_caracteristicas(tipo_vehiculo) doc = fitz.open(PLANTILLA_PDF) page = doc[0]
...
@app.route('/eliminar/<folio>') def eliminar(folio): supabase.table('permisos_guerrero').delete().eq('folio', folio).execute() flash('Registro eliminado exitosamente.', 'success') return redirect(url_for('panel'))

@app.route('/regenerar_pdf/<folio>') def regenerar_pdf(folio): registro = supabase.table('permisos_guerrero').select('*').eq('folio', folio).single().execute().data
