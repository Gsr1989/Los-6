...
PDF_OUTPUT_FOLDER = 'static/pdfs' PLANTILLA_PDF = 'Guerrero.pdf' FOLIO_FILE = 'data/folio_actual.txt' REGISTRO_FILE = 'data/registros.txt'

USUARIO_VALIDO = "elwarrior" CONTRASENA_VALIDA = "Warrior2025"

def cargar_folio(): if not os.path.exists(FOLIO_FILE): with open(FOLIO_FILE, 'w') as f: f.write('AA0001') with open(FOLIO_FILE, 'r') as f: return f.read().strip()

def siguiente_folio(folio_actual): letras = folio_actual[:2] try: numeros = int(folio_actual[2:]) except ValueError: numeros = 0 if numeros < 9999: numeros += 1 else: letras = incrementar_letras(letras) numeros = 1 nuevo_folio = f"{letras}{numeros:04d}" with open(FOLIO_FILE, 'w') as f: f.write(nuevo_folio) return nuevo_folio
...
def guardar_en_supabase(folio, marca, linea, anio, serie, motor, color, contribuyente, fecha_exp, fecha_ven): supabase.table("borradores_registros").insert({ "folio": folio, "marca": marca, "linea": linea, "anio": anio, "numero_serie": serie, "numero_motor": motor, "color": color, "contribuyente": contribuyente, "fecha_expedicion": fecha_exp.isoformat(), "fecha_vencimiento": fecha_ven.isoformat() }).execute()

def guardar_en_txt(folio, marca, linea, anio, serie, motor, color, contribuyente, fecha_exp, fecha_ven): with open(REGISTRO_FILE, 'a', encoding='utf-8') as f: f.write(f"{folio}|{marca}|{linea}|{anio}|{serie}|{motor}|{color}|{contribuyente}|{fecha_exp}|{fecha_ven}\n")

def generar_pdf(folio, marca, linea, anio, serie, motor, color, contribuyente, fecha_exp, fecha_ven): doc = fitz.open(PLANTILLA_PDF) page = doc[0]
...
@app.route('/', methods=['GET', 'POST']) def login(): if request.method == 'POST': usuario = request.form['usuario'] contraseña = request.form['contraseña'] if usuario == USUARIO_VALIDO and contraseña == CONTRASENA_VALIDA: session['usuario'] = usuario return redirect(url_for('panel')) else: flash('Credenciales incorrectas', 'danger') return render_template('login.html')

@app.route('/panel') def panel(): if 'usuario' not in session: return redirect(url_for('login')) return render_template('panel.html')

@app.route('/formulario', methods=['GET', 'POST']) def formulario(): if 'usuario' not in session: return redirect(url_for('login'))
...
@app.route('/consultar', methods=['GET', 'POST']) def consultar(): if request.method == 'POST': folio = request.form['folio'].strip().upper() path = os.path.join(PDF_OUTPUT_FOLDER, f"{folio}.pdf") if os.path.exists(path): return send_file(path, as_attachment=True) else: flash("Folio no encontrado o no tiene PDF generado.", "danger") return render_template('reimprimir.html')

@app.route('/descargar/<folio>') def descargar(folio): path = os.path.join(PDF_OUTPUT_FOLDER, f"{folio}.pdf") if os.path.exists(path): return send_file(path, as_attachment=True) return "El archivo no existe", 404
