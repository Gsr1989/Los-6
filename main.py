...
def generar_pdf(folio, tipo_vehiculo, marca, linea, año, serie, motor, color, contribuyente, fecha_expedicion, fecha_vencimiento):
    ...
    page.insert_text((79, 392), f"NÚMERO DE SERIE: {serie}", fontsize=8)
    page.insert_text((76, 402), f"NÚMERO DE MOTOR: {motor}", fontsize=8)
    page.insert_text((93, 412), f"MARCA: {marca}", fontsize=8)
    page.insert_text((91, 422), f"MODELO: {linea}", fontsize=8)
    page.insert_text((100, 432), f"AÑO: {año}", fontsize=8)
    page.insert_text((100, 442), f"COLOR: {color}", fontsize=8)
